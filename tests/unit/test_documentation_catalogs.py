...
def test_engineering_reference_routes_and_tool_contracts_exist() -> None:
    """Check routing/API identifiers, not duplicate long policy prose in tests.

    Semantic counterexamples live in test_engineering_policy_experiments.py.
    """
    bundle = ROOT / ".agents/skills/freecad-engineering"
    skill = (bundle / "SKILL.md").read_text(encoding="utf-8")
    routes = {
        "model-editing.md": {"capture_shape_checkpoint", "compare_shape_checkpoint"},
        "sketch-construction.md": {"edit_sketch_geometry", "get_sketch_info"},
        "drawing-reconstruction.md": {"compare_images", "open_image_tiles"},
        "sheet-metal-flat-patterns.md": {"sheet_metal_capabilities"},
    }
    tools = {
        _registered_tool_name(node)
        for path in (ROOT / "src/freecad_mcp/tools").glob("*.py")
        for node in _decorated_functions(path, "tool")
    }
    for filename, contract_names in routes.items():
        assert f"(references/{filename})" in skill
        text = (bundle / "references" / filename).read_text(encoding="utf-8")
        assert contract_names <= tools
        for name in contract_names:
            assert name in text
    assert "acceptance_manifest" in skill
