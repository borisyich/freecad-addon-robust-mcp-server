...
async def _execute_python_result(tools: dict[str, Any], *, code: str) -> Any:
...
async def _base_plate(tools: dict[str, Any], doc: str, body: str = "Body") -> str:
    ...
    await _call(
        ...
        operations=[
            {"op": "add_rectangle", "x": -30, "y": -20, "width": 60, "height": 40}
        ],
        doc_name=doc,
    )
...
""",
    ...
    await _call(
        ...
        operations=[{"op": "add_circle", "center_x": x, "center_y": 0, "radius": 3}],
        doc_name=doc,
    )
    pocket = await _call(
        tools,
...
async def test_pocket_direction_follows_global_sketch_normal(
    ...
    await _call(
        ...
        operations=[{"op": "add_circle", "center_x": 0, "center_y": 0, "radius": 3}],
        doc_name=doc,
    )
    pocket = await _call(
        tools,
...
@pytest.mark.asyncio
async def test_sketch_geometry_and_constraint_operation_catalog(
    live_tools: dict[str, Any],
) -> None:
    tools = live_tools
...
@pytest.mark.asyncio
async def test_prismatic_feature_chain_and_choice_values(
    live_tools: dict[str, Any],
) -> None:
    tools = live_tools
...
""",
    ...
    multi_doc = "McpAuditPrismaticMultiTransform"
    await _plate_with_offset_pocket(tools, multi_doc)
    multi = await _call(
        tools,
        "multi_transform_pattern",
        feature_name="Pocket",
        transformations=[
            {"kind": "linear", "direction": "X", "length": 30, "occurrences": 2},
            {"kind": "linear", "direction": "Y", "length": 20, "occurrences": 2},
        ],
        name="Multi",
        doc_name=multi_doc,
    )
    assert multi["validated"] is True
    assert multi["transformation_validation"]["ok"] is True
    assert multi["transformation_validation"]["stage_count"] == 2
    assert all(
        stage["original_count"] == 0
        and stage["owned_by_multi_transform"] is True
        and stage["shape_is_null"] is True
        for stage in multi["transformation_validation"]["stages"]
    )
    await _assert_valid_model(tools, multi_doc)
    ...
    with pytest.raises(ValueError, match="will not switch to Part::Fillet"):
        await _call(
            tools,
            "fillet_edges",
            object_name="Body",
    ...
    with pytest.raises(ValueError, match="will not switch to Part::Chamfer"):
        await _call(
            tools,
            "chamfer_edges",
            object_name="Body",
            size=1.0,
            edges=["Edge1"],
            name="WrongBodyChamfer",
            doc_name=chamfer_doc,
        )
    chamfer = await _call(
        tools,
        "chamfer_edges",
...
@pytest.mark.asyncio
async def test_revolution_loft_sweep_and_subtractive_families(
    live_tools: dict[str, Any],
) -> None:
    tools = live_tools
...
""",
    ...
    thickness = await _call(
        ...
        object_name="BasePad",
        thickness=1,
        faces_to_remove=[top["face"]],
        name="Thickness",
        doc_name=thickness_doc,
    )
...
""",
    ...
    subloft = await _call(
        tools,
        "subtractive_loft",
        sketch_names=["LoftCutA", "LoftCutB"],
        ruled=False,
        closed=False,
        name="SubtractiveLoft",
        doc_name=subloft_doc,
    )
