...
@pytest.mark.asyncio
async def test_contracts_and_document_lifecycle(
...
async def test_generic_part_object_workflow(live_tools: dict[str, Any]) -> None:
    ...
    await _call(
        ...
        type_id="Part::Box",
        name="RawBox",
        properties={"Length": 20.0, "Width": 16.0, "Height": 12.0},
        doc_name=doc,
    )
    for kind, dimensions in {
        ...
        await _call(
            tools,
            "create_primitive",
            primitive={"kind": kind, **dimensions},
            name=f"P_{kind}",
            doc_name=doc,
        )
    await _call(
        ...
        "edit_object",
        object_name="RawBox",
        properties={"Length": 22.0},
        doc_name=doc,
    )
    await _call(
        tools,
        "set_placement",
        object_name="P_cylinder",
        position=[8, 3, 0],
        rotation=[0, 0, 10],
        doc_name=doc,
    )
    ...
    await _call(
        tools,
        "scale_object",
        object_name="P_sphere",
        scale=1.2,
        result_name="SphereScaled",
        doc_name=doc,
    )
    ...
    await _call(
        ...
        "boolean_operation",
        operation="common",
        object1_name="P_sphere",
        object2_name="P_cone",
        result_name="BoolCommon",
        doc_name=doc,
    )
    await _call(
        tools,
    ...
    await _call(
        tools,
        "make_wire",
        points=[[0, 0, 0], [12, 0, 0], [12, 8, 0], [0, 8, 0]],
        closed=True,
        name="Wire",
        doc_name=doc,
    )
    await _call(
        tools, "make_face", object_name="Wire", result_name="Face", doc_name=doc
    )
    await _call(
        tools,
        "extrude_shape",
    ...
    await _call(
        tools,
        "make_compound",
        object_names=["P_cone", "P_sphere"],
        result_name="Compound",
        doc_name=doc,
    )
...
""",
    ...
    await _call(
        tools,
        "part_loft",
        profile_names=["LoftA", "LoftB"],
        result_name="PartLoft",
        doc_name=doc,
    )
    await _call(
        tools,
        "part_sweep",
        profile_name="SweepProfile",
        spine_name="SweepSpine",
        result_name="PartSweep",
        doc_name=doc,
    )
...
async def test_spreadsheet_macro_export_image_gui_and_validation_workflow(
    ...
    await _call(
        tools, "spreadsheet_get_aliases", spreadsheet_name="Params", doc_name=doc
    )
    await _call(
        tools,
    ...
    await _call(
        ...
        "create_macro",
        name=macro,
        code="_result_ = {'value': audit_value}",
        description="audit",
    )
    ...
    for fmt in ("step", "iges", "stl", "3mf", "obj"):
        await _call(
            ...
            "export",
            file_format=fmt,
            file_path=str(tmp_path / f"box.{fmt}"),
            object_names=["Box"],
            doc_name=doc,
        )
    for fmt in ("step", "stl"):
    ...
    await _call(
        tools,
        "compare_images",
        reference_path=str(image_a),
        candidate_path=str(image_b),
    ...
    font_path = (
        fonts["fonts"][0]["path"]
        if isinstance(fonts, dict) and fonts.get("fonts")
        else None
    )
    shape = await _call(
        tools,
    ...
    for plane in ("XY_Plane", "XZ_Plane", "YZ_Plane"):
        await _call(
            ...
            shapestring_name=shape_name,
            plane=plane,
            sketch_name=f"TextSketch{plane[:2]}",
            doc_name=doc,
        )
    ...
    bench_names = (
        list(benches.get("workbenches", benches))
    ...
    camera_state = await _call(tools, "get_camera_state", doc_name=doc)
    assert camera_state["success"] is True
    ...
    await _call(
        ...
        checkpoint_name="box_before_direct_edit",
        object_name="Box",
        doc_name=doc,
    )
    await _call(
    ...
    assert difference["difference"]["available"] is True
    assert difference["difference"]["geometric_change"] is True
...
async def test_draft_shapestring_honors_explicit_doc_name(
    ...
    target_identifiers = {
        value for item in target_objects for value in (item["name"], item["label"])
    }
