...
try:
    ...
    if not valid_records:
        raise ValueError("Shape difference contains no valid solid components")
    signature_groups = {{}}
    ...
    for record in selected:
        ...
        if {refine!r}:
            try:
                ...
                if refined.isNull() or not refined.isValid():
                    raise ValueError("removeSplitter produced a null or invalid Shape")
                component_refine_geometry_guard = _geometry_preservation_check(
                    component_shape,
                    refined,
                    {max_volume_drift_absolute!r},
                    {max_volume_drift_relative!r},
...
try:
    ...
    healing_geometry_guard = _geometry_preservation_check(
        source_shape,
        healed,
        {max_volume_drift_absolute!r},
        {max_volume_drift_relative!r},
