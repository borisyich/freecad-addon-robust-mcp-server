...
- For SheetMetal-designed parts, place holes and cutouts in the source flat
  blank sketch before `create_sheet_metal_base`. They must be flat-domain
  geometry that bends with its panel. Do not append `PartDesign::Hole` after a
  native SheetMetal feature; the sketches generated by Unfold are verification
  output, not a second production feature history.
